                        ...
                        <div class="col-xl-9 col-xxl-9 col-lg-8">
                                ...
                                <div class="owl-carousel category-card owl-theme overflow-hidden nav-none">
                                    <div class="item">
                                        <div data-bs-toggle="modal" data-bs-target="#Modalstory" class="card w125 h200 d-block border-0 shadow-none rounded-xxxl bg-dark overflow-hidden mb-3 mt-3">
                                            <div class="card-body d-block p-3 w-100 position-absolute bottom-0 ms-4 bottomdescription-center">
                                                <a href="#" class="">
                                    ...
                                    <div class="item" v-for="(item,index) in stories.data" :key="index">
                                        <div  data-bs-target="#Modalstory" class="card w125 h200 d-block border-0 shadow-xss rounded-xxxl bg-gradiant-bottom overflow-hidden cursor-pointer mb-3 mt-3">
                                            <img style="height:100%;" :src="'http://127.0.0.1:8000/storage/'+item.image" alt="">
                                            <div class="card-body d-block p-3 w-100 position-absolute bottom-0 text-center">
                                                <a href="#">
                                                    <figure class="avatar ms-auto me-auto mb-0 position-relative w50 z-index-1"><img :src="'http://127.0.0.1:8000/storage/'+item.user_image" alt="image" style="width: 50px;height: 50px;border-radius: 100%;" class="float-right p-0 bg-white  shadow-xss"></figure>
                                                    <div class="clearfix"></div>
                                                    <h4 class="fw-600 position-relative z-index-1 ls-1 font-xssss text-white mt-2 mb-1">{{ item.user_name }} </h4>
                                                </a>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div class="card w-100 shadow-xss rounded-xxl border-0 ps-4 pt-4 pe-4 pb-3 mb-3">
                                <div class="card-body p-0">
                                    <div @click="create()" class=" font-xssss fw-600 text-grey-500 card-body p-0 d-flex align-items-center"><i class="btn-round-sm font-xs text-primary feather-edit-3 me-2 bg-greylight"></i>Create Post</div>
                                </div>
                                <div class="card-body p-0 mt-3 position-relative">
                                    <figure class="avatar position-absolute ms-2 mt-1 top-5"><img style="width: 30px;height: 30px;border-radius: 10%;" :src="'http://127.0.0.1:8000/storage/'+$page.props.data_user.image" alt="image" class="shadow-sm rounded-circle mt-1"></figure>
                                    <textarea v-model="post.description" name="message" class="h100 bor-0 w-100 rounded-xxl pt-3 ps-5 font-xssss text-grey-500 fw-500 border-light-md theme-dark-bg" cols="30" rows="10" placeholder="   What's on your mind?"></textarea>
                                </div>
                                <div class="card-body d-flex p-0 ">
                                    <a href="#" class="d-flex align-items-center font-xssss fw-600 ls-1 text-grey-700 text-dark pe-4"><i class="font-md text-danger feather-video me-2"></i><span class="d-none-xs">Live Video</span></a>
                                        <input @change="uploadimg"   type="file" class="d-flex align-items-center mt-2 ms-5 font-xssss fw-600 ls-1 text-grey-700 text-dark pe-4">
                            ...
                            <div v-for="(post,index) in posts.data" :key="index" class="card w-100 shadow-xss rounded-xxl border-0 p-4 mb-3">
                                <div class="card-body p-0 d-flex">
                                    <figure class="avatar me-3"><img :src="'http://127.0.0.1:8000/storage/'+post.user_image" alt="image" class="shadow-sm rounded-circle" style="width: 30px;height:30px;" ></figure>
                                    <h4 class="fw-700 text-grey-900 font-xssss mt-1"><Link :href="'/user/profile/'+post.user_id">{{ post.user_name }}</Link><span class="d-block font-xssss fw-500 mt-1 lh-3 text-grey-500">3 hour ago</span></h4>
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end p-4 rounded-xxl border-0 shadow-lg" aria-labelledby="dropdownMenu2">
                                        ...
                                        <div v-if="post.user_id==$page.props.data_user.id" class="card-body p-0 d-flex mt-2">
                                            <i class="feather-alert-circle text-grey-500 me-3 font-lg"></i>
                                            <h4 class="fw-600 text-grey-900 font-xssss mt-0 me-4"><Link :href="'/user/profile/edit/'+post.id">Edit Post</Link> <span class="d-block font-xsssss fw-500 mt-1 lh-3 text-grey-500">Save to your saved items</span></h4>
                                        </div>
                                ...
                                <div class="card-body d-block p-0">
                                    <div class="row ps-2 pe-2">
                                        <div v-if="post.image" class="col-lg-12 col-sm-4 p-1"><a href="" ><img :src="'http://127.0.0.1:8000/storage/'+post.image"  alt="image"></a></div>
                                    </div>
                                </div>
                                <div class="card-body d-flex p-0 mt-3">
                                    <div  class="emoji-bttn d-flex align-items-center fw-600 text-grey-900 text-dark lh-26 font-xssss me-2"><i  @click="like(post.id,post.user_id)"  class="feather-heart text-white bg-red-gradiant me-2 btn-round-xs font-xss"></i> <span v-for="(like,index) in likes" :key="index"><strong v-if="like.post_id==post.id" class="me-1" >{{ like.count }}</strong></span> Likes</div>
                                    <Link :href="'/comment/'+post.id" class="d-flex align-items-center fw-600 text-grey-900 text-dark lh-26 font-xssss"><i class="feather-message-circle text-dark text-grey-900 btn-round-sm font-lg"></i><span class="d-none-xss"><span v-for="(com,index) in comments" :key="index"><strong v-if="com.post_id==post.id" class="me-1" >{{ com.count }}</strong></span> Comment</span></Link>
...
export default {
props:{user_list:Object,notifications:Array,posts:Object,likes:Array,comments:Array,stories:Object},
...
methods:{
    ...
    create()
    {
        ...
        this.$inertia.post("/post/create",this.post);
    },
    like(id,post_user_id)
    {
